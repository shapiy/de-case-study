...
class AggregationRequestSerializer(serializers.Serializer):
    row_fields = serializers.CharField(
        required=True, help_text="Field to group rows by (e.g., 'transaction_type')"
    )
    column_fields = serializers.CharField(
        required=True, help_text="Comma-separated fields for columns (e.g., 'year,status')"
    )
        ...
        for field in fields:
            if field not in allowed:
                raise serializers.ValidationError(
                    f"All fields must be one of: {', '.join(allowed)}"
                )
